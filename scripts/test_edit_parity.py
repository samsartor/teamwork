...
import teamwork.pipeline_flux2_klein  # registers FLUX2 / FLUX2_PLUSATTN
...
def teamwork_edit_prediction(adapted, gen, ref, text, hh, ww, offset, device):
    ...
    Per-teammate T-coord = teammate * offset. Returns the edited.out prediction.
    """
    L_img = hh * ww
    L_text = text.shape[1]
    T = 2
    ...
    # Per-teammate image ids: teammate k -> T = k * offset (pipeline uses offset=1).
    ids = torch.cat(
        [img_ids(hh, ww, k * offset, device) for k in range(T)], dim=0
    )  # (T*L_img, 4)
...
def main():
    ...
    with torch.no_grad():
        ...
        for offset, label in [(1, "current (teammate*1)"), (KLEIN_REF_SCALE, "klein ref (teammate*10)")]:
            tw_pred = teamwork_edit_prediction(adapted, gen, ref, text, hh, ww, offset, device)
            diff = (ref_pred.float() - tw_pred.float()).abs().max().item()
            status = "PASS" if diff <= tol else "FAIL"
            print(f"[{status}] offset={offset:2d} ({label}): "
                  f"max-abs-diff vs base editing = {diff:.3e}")
